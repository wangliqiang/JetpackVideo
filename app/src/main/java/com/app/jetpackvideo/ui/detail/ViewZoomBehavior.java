...
public class ViewZoomBehavior extends CoordinatorLayout.Behavior<FullScreenPlayerView> {
    ...
    private ViewDragHelper.Callback mCallback = new ViewDragHelper.Callback() {
        ...
        // 告诉ViewDragHelper手指拖拽这个View，本次滑动最终能够滑动的距离
        @Override
        public int clampViewPositionVertical(@NonNull View child, int top, int dy) {
            ...
            if (dy < 0 && refChild.getBottom() < minHeight
                    ...
                    // 也意味围着列表还可以向下滑动，此时应该让列表自行滑动，不做拦截
            ...
            if (dy > 0) {
                ...
                //else 分支里面 dy的值 是负值。
                //如果本次滑动的值  dy 加上refChild的bottom 会小于minHeight。
                //那么应该计算一下本次能够滑动的最大距离
            ...
            if (viewZoomCallback != null) {
                viewZoomCallback.onDragZoom(layoutParams.height);
            }
            return maxConsumed;
        }
    ...
    private ViewZoomCallback viewZoomCallback;

    public void setViewZoomCallback(ViewZoomCallback callback) {
        this.viewZoomCallback = callback;
    }
        ...
        public void run() {
            ...
            if (overScroller.computeScrollOffset() && height >= minHeight && height <= childOriginalHeight) {
                ...
                if (newHeight != height) {
                    ...
                    if (viewZoomCallback != null) {
                        viewZoomCallback.onDragZoom(newHeight);
                    }
                }
